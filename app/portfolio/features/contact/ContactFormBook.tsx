...
const ContactFormBook = () => {
  ...
  return (
    <section className='contact__form__step contactFormBooking' data-toggle='false'>
      ...
      <div className='contact__form__step__calendar'>
        ...
        <ul className='contact__form__step__calendar__weekdays'>
          {weekdays.map((day) => (
            <li key={`weekday-${day}`} className='contact__form__step__calendar__weekdays__day'>
              {day}
            </li>
          ))}
        </ul>
        <ul className='contact__form__step__calendar__selection'>
          {emptyDays.map((_, i) => (
            <li key={`empty-${i}`} className='contact__form__step__calendar__selection__opt' />
          ))}
          {daysInMonth.map((date) => {
            const isPast = date < today;
            const isSelected = selectedDate && selectedDate.toDateString() === date.toDateString();
            return (
              <li key={date.toDateString()} className='contact__form__step__calendar__selection__opt'>
                <button
                  type='button'
      ...
      />
      {errors['bookingDate'] && (
        <div id='bookingDate-error' className='contact__form__step__ul__li__error' role='alert'>
          {errors['bookingDate']}
        </div>
      )}
